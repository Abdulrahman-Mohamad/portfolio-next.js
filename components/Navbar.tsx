      ...
      <header className={`navbar ${scrolled ? "scrolled" : "not-scrolled"} z-40`}>
        <div className="inner">
          ...
          <Link href="/contact" className="contact-btn group hidden lg:block">
            <div className="inner px-5 py-2 rounded-lg bg-white text-black transition-colors duration-300 hover:bg-black-50 group-hover:bg-black-50">
        ...
        >
          <div className="flex flex-col h-full space-y-10">
            <div
              ...
              className="text-white-50 text-xl font-semibold flex items-center gap-4"
            >
              <Link href="/" className="flex items-center gap-4" onClick={handleCloseMenu}>
                <Image src="/favIcon.png" alt="Logo" width={60} height={60} className="w-15 h-auto" />
                AbdelRahman
              </Link>
            </div>
            ...
            <ul className="space-y-6">
              {navLinks.map(({ link, name }, index) => (
                <li
                  key={name}
                  ref={(el) => { if (el) menuItemsRef.current[index + 1] = el; }}
                  className="group text-white-50 relative"
                >
